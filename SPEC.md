...
- `.codex/`：`solo init` 安装的 Codex 项目级 hook 配置。
...
- `.solo-runtime/`：SOLO 本地运行时根目录，默认应加入 `.gitignore`。
- `.solo-runtime/projects/{projectId}/`：某个 Managed Project 的独立运行时目录。
- `.solo-runtime/projects/{projectId}/db.sqlite`：SOLO 对该项目的本地缓存，不是项目真实状态来源。
...
```json
{
  ...
  "runtimeDir": ".solo-runtime/projects/{projectId}",
  "createdAt": "datetime",
  "updatedAt": "datetime"
}
```
...
`firstPrompt` 只记录该 session 首次 `UserPromptSubmit` 的提示词摘要，按空白折叠后最多 80 个字符。
...
| `GET` | `/api/projects/:id/codex-sessions` | 获取 Codex hook 记录的 session 列表 |
| `POST` | `/api/projects/:id/init` | 安装或更新项目级 Codex hooks |

SOLO UI 不提供项目选择或打开项目入口。当前 Managed Project 仅由启动时的 `solo serve --repo` 或 `solo serve /path/to/repo` 决定。
...
| `GET` | `/api/projects/:id/workflows/:workflowId` | 获取 workflow 定义 |
| `POST` | `/api/projects/:id/workflows/bootstrap` | 使用 Codex 创建或更新 workflow |
| `POST` | `/api/projects/:id/workflows/:workflowId/status` | 执行一次状态查询 |
| `GET` | `/api/projects/:id/workflows/:workflowId/items` | 获取 workflow 报告的 Work Item |
| `GET` | `/api/projects/:id/workflows/:workflowId/views` | 获取 workflow 声明的 UI 视图 |

### Action API

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `POST` | `/api/projects/:id/workflows/:workflowId/actions/:actionId/run` | 执行 workflow action |
| `GET` | `/api/projects/:id/runs` | 获取项目 run 列表 |
| `POST` | `/api/projects/:id/runs/:runId/stop` | 停止正在运行的 action |
| `GET` | `/api/projects/:id/runs/:runId` | 获取 run 详情 |
| `GET` | `/api/projects/:id/runs/:runId/logs` | 获取 run 日志 |
| `GET` | `/api/projects/:id/runs/:runId/final` | 获取 Codex 最终输出 |
| `GET` | `/api/projects/:id/runs/:runId/diff` | 获取 run 对应 scope 的 git diff |
| `POST` | `/api/projects/:id/runs/:runId/open-editor` | 在 run 的 scope 中打开 code-server |
...
`solo init` 在 Managed Project 内安装 Codex 项目级 hook：

- `.codex/config.toml`：启用 `features.codex_hooks = true`。
- `.codex/hooks.json`：为 `SessionStart`、`UserPromptSubmit`、`Stop` 注册当前 Python 的 `-m solo.codex_hook`。

hook 写入 `.solo-runtime/projects/{projectId}/db.sqlite` 的 `codex_sessions` 缓存表：

- 每个 Codex session 只保留第一句 prompt 摘要，最长 80 字符。
- `UserPromptSubmit` 会递增 `turnCount`。
- `Stop` 将 session 标记为 `ended`。
- hook 失败不能阻断 Codex 正常执行。
...
- 完整 prompt 写入 `.solo-runtime/projects/{projectId}/prompts/{runId}.prompt.txt`。
- stdout 和 stderr 写入 `.solo-runtime/projects/{projectId}/logs/{runId}.log`。
- Codex 最终输出写入 `.solo-runtime/projects/{projectId}/logs/{runId}.final.txt`。
...
```bash
code-server \
  --config {runtimeDir}/editors/{editorId}/config.yaml \
  --bind-addr 127.0.0.1:{port} \
  --auth none \
  --ignore-last-opened \
  --user-data-dir {runtimeDir}/editors/{editorId}/user-data \
  --extensions-dir {runtimeDir}/editors/{editorId}/extensions \
  --session-socket /tmp/solo-{editorId}.sock \
  {scopePath}
```
...
- 端口由后端自动分配并记录在 `.solo-runtime/projects/{projectId}/editors/` 中。
- code-server 必须优先使用 SOLO 自带的 `.tools/code-server`；只有不存在时才允许使用非 VS Code Server 环境中的全局 `code-server`。
- 启动 code-server 前必须清理 `VSCODE_*`、`ELECTRON_*`、`CODE_SERVER_SESSION_SOCKET` 等环境变量，并从 `PATH` 移除 `.vscode-server`、`.cursor-server`、`.windsurf-server` 等远程 IDE 目录，避免打开宿主 VS Code 环境。
- 每个 editor 使用独立的 `config.yaml`、`user-data-dir`、`extensions-dir` 和短路径 `session-socket`；不得读取用户全局 code-server 配置决定端口或认证。
- `open-editor` 只有在分配端口实际可连接后才返回 URL；启动失败时返回 code-server 日志摘要。
...
- `editor-*` Host 由 SOLO 后端反向代理到对应的本地 `127.0.0.1:{port}`，只允许代理当前 Managed Project 的 `.solo-runtime/projects/{projectId}/editors/` 中记录且进程仍存活的实例。
...
- 后端初始化 `.solo-runtime/projects/{projectId}/` 和 SQLite 缓存。
...
- 过程日志写入 `.solo-runtime/projects/{projectId}/logs/{runId}.log`。
- 最终输出写入 `.solo-runtime/projects/{projectId}/logs/{runId}.final.txt`。
- 执行结束后 SOLO 重新查询项目状态。
